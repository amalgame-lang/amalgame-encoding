#!/bin/bash
# ─────────────────────────────────────────────────────
#  amalgame-framework-encoding — Test Runner
...
# ── Setup ──────────────────────────────────────────────
BUILD_DIR="$(mktemp -d -t afenc-tests-XXXXXX)"
...
echo ""
echo "════════════════════════════════════════════"
echo "  amalgame-framework-encoding — Test Suite"
echo "════════════════════════════════════════════"
...
PKG_GIT="github.com/amalgame-lang/amalgame-framework-encoding"
...
[[package]]
name = "amalgame-framework-encoding"
git  = "$PKG_GIT"
tag  = "$PKG_TAG"
rev  = "$FAKE_SHA"
EOF
...
ARCHIVE="$FACADE_BUILD_DIR/libamalgame-pkg-Encoding.a"

# ── Pre-build the facade archive (mimics PrecompileFacade) ──
echo "── Pre-compiling facade.am → libamalgame-pkg-Encoding.a ──"
"$AMC" --lib --quiet "$PKG_ROOT/facade.am" -o "$FACADE_BUILD_DIR/Encoding-facade"
gcc -O2 -I"$AMC_RUNTIME" -w -c "$FACADE_BUILD_DIR/Encoding-facade.c" \
    -o "$FACADE_BUILD_DIR/Encoding-facade.o"
ar rcs "$ARCHIVE" "$FACADE_BUILD_DIR/Encoding-facade.o"
...
run_test() {
    ...
    cp "$SCRIPT_DIR/stdlib_encoding.am" "$PROJ_DIR/test.am"
...
run_test "b64 encode Hello"             "[PASS] b64 encode Hello"
run_test "b64 round trip"               "[PASS] b64 round trip"
run_test "b64 IsValid"                  "[PASS] b64 IsValid true"
run_test "hex encode"                   "[PASS] hex encode AB"
run_test "hex decode"                   "[PASS] hex decode AB"
run_test "url encode space"             "[PASS] url encode space"
run_test "url decode space"             "[PASS] url decode space"
